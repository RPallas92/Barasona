...
/// deserialize their data as it goes through Barasona. Instead, applications can present their data
/// models as-is to Barasona, Barasona will present it to the application's `BarasonaStorage` impl
///  when ready, and the application may then deal with the data directly in the storage engine
/// without having to do a preliminary deserialization.
...
/// their storage layer, up through Barasona, and back into their application for returning
/// data to clients.
///
...
/// the Barasona replication protocol.
