...
/// A result type where the error variant is always a `BarasonaError`.
pub type BarasonaResult<T> = std::result::Result<T, BarasonaError>;

/// Error variants related to the internals of Barasona.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum BarasonaError {
    /// An error which has come from the `BarasonaStorage` layer.
    #[error("{0}")]
    BarasonaStorage(anyhow::Error),
    /// An error which has come from the `BarasonaNetwork` layer.
    #[error("{0}")]
    BarasonaNetwork(anyhow::Error),
    /// An internal Barasona error indicating that Barasona is shutting down.
    #[error("Barasona is shutting down")]
    ShuttingDown,
}

impl From<tokio::io::Error> for BarasonaError {
    fn from(src: tokio::io::Error) -> Self {
        BarasonaError::BarasonaStorage(src.into())
    }
}
...
pub enum ClientReadError {
    /// A Barasona error.
    #[error("{0}")]
    BarasonaError(#[from] BarasonaError),
...
impl<D: AppData> fmt::Debug for ClientWriteError<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ClientWriteError::BarasonaError(err) => {
                f.debug_tuple("BarasonaError").field(err).finish()
            }
...
/// The set of errors which may take place when initializing a pristine Barasona node.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum InitializeError {
    ...
    #[error("{0}")]
    BarasonaError(#[from] BarasonaError),
    /// The requested action is not allowed due to the Barasona node's current state.
    #[error("the requested action is not allowed due to the Barasona node's current state")]
    NotAllowed,
}
...
pub enum ChangeConfigError {
    ...
    /// to the Barasona log and the process related to that workflow.
    #[error("{0}")]
    BarasonaError(#[from] BarasonaError),
    ...
    #[error("this node is not the Barasona leader")]
...
impl<D: AppData> From<ClientWriteError<D>> for ChangeConfigError {
    fn from(src: ClientWriteError<D>) -> Self {
        match src {
            ClientWriteError::BarasonaError(err) => Self::BarasonaError(err),
