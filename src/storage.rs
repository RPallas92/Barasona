//! The Barasona storage interface and data types.
...
use crate::{
    barasona::{Entry, MembershipConfig},
...
/// A record holding the persistent state of a Barasona node.
///
...
/// A struct used to represent the initial state which a Barasona node needs when first starting.
#[derive(Clone, Debug)]
...
impl InitialState {
    /// Create a new instance for a brand new Barasona node.
    ///
    /// ### `id`
...
/// A trait defining the interface for a Barasona storage system.
#[async_trait]
pub trait BarasonaStorage<D, R>: Send + Sync + 'static
where
    D: AppData,
    R: AppDataResponse,
{
    ...
    /// The error type used to indicate to Barasona that shutdown is needed when calling the
    ...
    ///
    /// For all other methods of this trait, returning an error will cause Barasona to shutdown.
    ...
    ///
    /// Errors returned from this method will cause Barasona to go into shutdown.
    async fn get_membership_config(&self) -> Result<MembershipConfig>;

    /// Get Barasona's state information from storage.
    ///
    /// When the Barasona node is first started, it will call this interface on the storage system to
    ...
    /// the node's persistent state record; and the index of the last log applied to the state machine.
    ///
    /// Errors returned from this method will cause Barasona to go into shutdown.
    async fn get_initial_state(&self) -> Result<InitialState>;

    /// Save Barasona's persistent state.
    ///
    /// Errors returned from this method will cause Barasona to go into shutdown.
    async fn save_persistent_state(&self, state: &PersistentState) -> Result<()>;
    ...
    /// should live. This is 100% application specific. This may be where a key/value
    /// is being stored.
    ///
    ...
    /// `BarasonaStorage::ShutdownError`, then Barasona will go into shutdown in order to preserve the
    /// safety of the data and avoid corruption. Any other errors will be propagated back up to the
    /// `Barasona.client_write` call point.
    ///
    ...
    /// the last applied log and under write load this value may change quickly. The storage system will
    /// perform a compaction that covers from start to a fixed last applied log, even it that last
    /// applied log changes.
    ///
