...
use crate::barasona::{ChangeMembershipTx, ClientWriteRequest, MembershipConfig};
use crate::core::{AppData, AppDataResponse, BarasonaNetwork, BarasonaStorage, NodeId, NonVoterState, State, LeaderState};
use crate::error::{BarasonaError, ChangeConfigError, InitializeError};
use crate::replication::BarasonaEvent;

use super::client::ClientRequestEntry;
use super::{ConsensusState, NonVoterReplicationState};
    ...
    NonVoterState<'a, D, R, N, S>
{
    ...
    pub(super) async fn handle_init_with_config(
        ...
        if self.core.last_log_index != 0 || self.core.last_log_term != 0 {
            tracing::error!({self.core.last_log_index, self.core.last_log_term}, 
                "rejecting init_with_config as last_log_index or last_log_term is not 0");
...
impl<'a, D: AppData, R: AppDataResponse, N: BarasonaNetwork<D>, S: BarasonaStorage<D, R>> LeaderState<'a, D, R, N, S> {
    ...
    pub(super) async fn change_membership(&mut self, members: HashSet<NodeId>, tx: ChangeMembershipTx) {
        ...
        let (tx_joint_entry_committed, rx_join_entry_committed) = oneshot::channel();
        ...
        let cr_entry = ClientRequestEntry::from_entry(entry, tx_joint_entry_committed);
        self.replicate_client_request(cr_entry).await;
        self.core.report_metrics();

        // Set up channels for eventual response to the 2-phase config change.
        ...
        self.joint_consensus_cb.push_back(rx_join_entry_committed); // Receiver for when the joint consensus is committed.
        tokio::spawn(async move {
            let res = rx_cfg_change
            .map_err(|_| BarasonaError::ShuttingDown)
            .into_future()
            .then(|res| {
    ...
    #[tracing::instrument(level = "trace", skip(self))]
    pub(super) async fn handle_joint_consensus_committed(&mut self) -> Result<(), BarasonaError> {
    ...
    /// Finalize the commited joint consensus
    #[tracing::instrument(level = "trace", skip(self))]
    pub(super) async fn finalize_joint_consensus(&mut self) -> Result<(), BarasonaError> {
        ...
        // The Barasona spec accounts for this using the 3rd safety measure of cluster configuration changes.
        // This measure is already implemented and in place.

        // Propagate the next command as any other client request.
        let payload: ClientWriteRequest<D> = ClientWriteRequest::<D>::new_config(self.core.membership.clone());
        let (tx_uniform_entry_committed, rx_uniform_entry_committed) = oneshot::channel();
        let entry = self.append_payload_to_log(payload.entry).await?;
        let cr_entry = ClientRequestEntry::from_entry(entry, tx_uniform_entry_committed);
        self.replicate_client_request(cr_entry);
        self.core.report_metrics();

        // Setup channel for eventual commitment of the uniform consensus config.
        self.uniform_consensus_cb.push_back(rx_uniform_entry_committed); // Receiver for when the uniform consensus is committed.
        Ok(())
    }

        /// Handle the commitment of a uniform consensus cluster configuration.
        #[tracing::instrument(level = "trace", skip(self))]
        pub(super) async fn handle_uniform_consensus_committed(&mut self, index: u64) -> Result<(), BarasonaError> {
            // Step down if needed
            if self.is_stepping_down {
                tracing::debug!("Barasona node is stepping down");
            ...
            let nodes_to_remove: Vec<_> = self.nodes.iter()
            ...
            for (_, replstate) in self.nodes.iter_mut() {
                if replstate.match_index < index {
            ...
            for node in nodes_to_remove {
                ...
                if let Some(node) = self.nodes.remove(&node) {
                    let _ = node.replication_stream.repl_tx.send(BarasonaEvent::Terminate);
                }
            }
            self.core.report_metrics();
            Ok(())
