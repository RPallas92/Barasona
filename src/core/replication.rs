...
use crate::core::{
    AppData, AppDataResponse, BarasonaResult, ConsensusState, LeaderState, NodeId,
    ReplicationState, State,
};
use crate::network::BarasonaNetwork;
use crate::replication::{BarasonaEvent, ReplicaEvent, ReplicationStream};
use crate::storage::{BarasonaStorage, CurrentSnapshotData};
    ...
    LeaderState<'a, D, R, N, S>
{
    ...
    async fn handle_rate_update(
        ...
        // Get a handle on the target's replication state and update it as needed
    ...
    async fn handle_revert_to_follower(&mut self, _: NodeId, term: u64) -> BarasonaResult<()> {
        if term > self.core.current_term {
            self.core.update_current_term(term, None);
            self.core.save_hard_state().await?;
            self.core
                .update_current_leader(super::UpdateCurrentLeader::Unknown);
    ...
    async fn handle_update_match_index(
        ...
        match self.nodes.get_mut(&target) {
            Some(state) => {
                ...
                if let Some(threshold) = state.remove_after_commit {
                    needs_removal = match_index >= threshold;
                }
            }
        ...
        if has_new_commit_index {
            self.core.commit_index = min(commit_index_c0, commit_index_c1);
            ...
            for node in self.nodes.values() {
                let _ = node
                    .replication_stream
                    .repl_tx
                    .send(BarasonaEvent::UpdateCommitIndex {
            ...
            // Check if there are any pending requests which need to be marked as committed.
            let filter = self
                .awaiting_committed
                .iter()
                .enumerate()
                .take_while(|(idx, elem)| elem.entry.index <= self.core.commit_index)
                .last()
                .map(|idx, _| idx);
...
/// - `current_commit`: is the Barasona node's `current_commit` value before invoking this function.
...
fn snapshot_is_within_half_of_threshold(
    ...
    let distance_from_line = last_log_index.saturating_sub(*snapshot_last_index); // Guard against underflow.
    let half_of_threshold = threshold / 2;
    distance_from_line <= half_of_threshold
}

// TODO Ricardo this is completed. Continue somewhere else. Maybe mod? Search TODO Ricardo
